...
//Code Here
let evenNumbers  = mixedNumbers.filter(function(elem,index,arr){
    if(!(elem % 2) === 0){
      return arr.elem
    } else {
      delete arr.elem
...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(elem,index,arr){
  if(!elem.CP < 200){
    return elem
  }
})
...
let orderTotals = orders.map(function(element,index,wholeArray){
...
let bobsTotal  = populations.reduce(function(runningTotal, curElement, curIndex, wholeArray){
  if (purchases.curElement.owner === "Bob"){
    runningTotal + curElement
  }
  return runningTotal
})
